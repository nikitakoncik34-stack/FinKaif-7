...
function parseTxFallback(raw) {
  ...
  // 1. Compound Russian Number Recognizer
...
КАТЕГОРИИ:
- Животные/Питомцы (корова, скот, ферма, бык, коза, свинья, лошадь, собака, кот, корм, ветеринар) -> "Питомцы"
- Еда/Кафе/Фастфуд (шава, шаверма, бургер, кофе, ресторан, пицца, доставка, суши, бар) -> "Кафе" или "Рестораны"
- Продукты (магазин, пятерочка, перекресток, еда, мясо, молоко) -> "Продукты"
- Транспорт (такси, карш, бензин, заправка, метро, проезд) -> "Транспорт"
- Техника (плойка, айфон, макбук, комп, видяха, гаджет) -> "Техника"
- Подписки (яндекс плюс, спотик, впн, телега, прем, нетфликс) -> "Подписки"
- Покупки/Одежда (шмот, кроссы, педали, вб, озон, зарина, лайм) -> "Покупки"
- Здоровье (зал, аптека, зубы, врач, таблетки, анализы) -> "Здоровье"
- Жилье (аренда, хата, жкх, коммуналка, ремонт, стройка) -> "Жилье"
- Развлечения (кино, стим, игры, донат, концерт, квест) -> "Развлечения"
- Доходы (зп, аванс, получка, премия, фриланс, продал, упали деньги, капнуло) -> "Зарплата" или "Фриланс" или "Инвестиции"

СУММЫ И СЛЕНГ:
- миллион, лям, лимон = 1 000 000
- полтора миллиона = 1 500 000
- миллиард, ярд, арбуз = 1 000 000 000
- 1кк = 1 000 000, 2.5кк = 2 500 000
- сотка при доходе / поступлении = 100 000, сотка рублей = 100
- полтос = 50 000, сорокет = 40 000, косарь/кусок/штука = 1 000, пятихатка = 500, двушка = 2000, трешка = 3000, чирик = 10000

ДАТЫ: сегодня = ${toIso(now)}, вчера = ${toIso(dYesterday)}, позавчера = ${toIso(dBefore)}

Ответь СТРОГО JSON без markdown:
{"type":"income"|"expense","category":"Категория","amount":число,"description":"Описание","occurred_on":"YYYY-MM-DD"}`;
